 ...
 * @author dyoung
 *         <p/>
 *         Differences from Apple's SDK: 1. You can wildcard all fields in a
 *         region to get updates about ANY iBeacon 2. Ranging updates don't come
 *         as reliably every second. 3. The distance measurement algorithm is
 *         not exactly the same 4. You can do ranging when the app is not in the
 *         foreground 5. It requires Bluetooth Admin privileges
 *         <p/>
 *         Open Issues: 1. If an activity/service unbinds after staring
 *         monitoring or ranging, we will continue to make callbacks from the
 *         service 2. Is sending so many intents efficient?
 */
...
public class IBeaconService extends Service {
	...
	public void startRangingBeaconsInRegion(Region region, Callback callback) {
		if (rangedRegionState.containsKey(region)) {
			...
			rangedRegionState.remove(region); // need to remove it, otherwise
												// the old object will be
												// retained because they are
												// .equal
		}
	...
	public void startMonitoringBeaconsInRegion(Region region, Callback callback) {
		...
		if (monitoredRegionState.containsKey(region)) {
			...
			monitoredRegionState.remove(region); // need to remove it, otherwise
													// the old object will be
													// retained because they are
													// .equal
		}
