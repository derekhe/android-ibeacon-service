...
public class RangedIBeacon extends IBeacon {
	...
	private double calculateRunningAverage() {
		...
		if (IBeaconManager.LOG_DEBUG)
			Log.d(TAG, "Running average rssi based on " + size
					+ " measurements: " + runningAverage);
	...
	protected void addRangeMeasurement(Integer rssi) {
		...
		if (IBeaconManager.LOG_DEBUG)
			Log.d(TAG,
					"calculating new range measurement with new rssi measurement:"
							+ rssi);
		runningAverageRssi = calculateRunningAverage();
		accuracy = null; // force calculation of accuracy and proximity next
