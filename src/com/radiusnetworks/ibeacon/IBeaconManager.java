...
public class IBeaconManager {
	...
	/**
	 * Sets the duration in milliseconds to wait between each bluetooth scan
	 * cycle used to look for iBeacons
	 * 
	 * @param p
	 */
	...
	/**
	 * Sets the duration in milliseconds of each Bluetooth LE scan cycle to look
	 * for iBeacons when no ranging/monitoring clients are in the foreground
	 * 
	 * @param p
	 */
	...
	public void bind(IBeaconConsumer consumer) {
		if (consumers.keySet().contains(consumer)) {
			...
			if (serviceMessenger != null) { // If the serviceMessenger is not
											// null, that means we are able to
											// make calls to the service
				setBackgroundMode(consumer, false); // if we just bound, we
													// assume we are not in the
													// background.
			}
		}
	}
