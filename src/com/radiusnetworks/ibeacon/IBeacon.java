...
/**
 * The <code>IBeacon</code> class represents a single hardware iBeacon detected
 ...
 * </pre>
 * 
 * An iBeacon sends a Bluetooth Low Energy (BLE) advertisement that contains
 * these three identifiers, along with the calibrated tx power (in RSSI) of the
 * iBeacon's Bluetooth transmitter.
 * 
 * This class may only be instantiated from a BLE packet, and an RSSI
 * measurement for the packet. The class parses out the three part identifier,
 * along with the calibrated tx power. It then uses the measured RSSI and
 * calibrated tx power to do a rough distance measurement (the accuracy field)
 * and group it into a more reliable buckets of distance (the proximity field.)
 * 
 * @author David G. Young
...
public class IBeacon {
	...
	/**
	 * No distance estimate was possible due to a bad RSSI value or measured TX
	 * power
	 */
	...
	/**
	 * A 16 byte UUID that typically represents the company owning a number of
	 * iBeacons Example: E2C56DB5-DFFB-48D2-B060-D0F5A71096E0
	 */
	...
	protected int minor;
	/**
	 * An integer with four possible values representing a general idea of how
	 * far the iBeacon is away
	 * 
	 ...
	 * This name is confusing, but is copied from the iOS7 SDK terminology. Note
	 * that this number fluctuates quite a bit with RSSI, so despite the name,
	 * it is not super accurate. It is recommended to instead use the proximity
	 * field, or your own bucketization of this value.
	 */
	protected Double accuracy;
	/**
	 * The measured signal strength of the Bluetooth packet that led do this
	 * iBeacon detection.
	 */
	protected int rssi;
	/**
	 * The calibrated measured Tx power of the iBeacon in RSSI This value is
	 * baked into an iBeacon when it is manufactured, and it is transmitted with
	 * each packet to aid in the distance estimate
	 */
	...
	/**
	 * Used to attach data to individual iBeacons, either locally or in the
	 * cloud
	 */
	...
	public double getAccuracy() {
		if (accuracy == null) {
			accuracy = calculateAccuracy(txPower,
	...
	/**
	 * Two detected iBeacons are considered equal if they share the same three
	 * identifiers, regardless of their distance or RSSI.
	 */
	@Override
	public boolean equals(Object that) {
		...
		return (thatIBeacon.getMajor() == this.getMajor()
				&& thatIBeacon.getMinor() == this.getMinor() && thatIBeacon
				.getProximityUuid().equals(this.getProximityUuid()));
	}

	/**
	 * Construct an iBeacon from a Bluetooth LE packet collected by Android's
	 * Bluetooth APIs
	 * 
	 * @param scanData
	...
	public static IBeacon fromScanData(byte[] scanData, int rssi) {
		...
		while (startByte <= 5) {
			if (((int) scanData[startByte] & 0xff) == 0x4c
				...
				if (IBeaconManager.LOG_DEBUG)
					Log.d(TAG,
							"This is a proprietary Estimote beacon advertisement that does not meet the iBeacon standard.  Identifiers cannot be read.");
				...
				if (IBeaconManager.LOG_DEBUG)
					Log.d(TAG,
							"This is a proprietary Gimbal beacon advertisement that does not meet the iBeacon standard.  Identifiers cannot be read.");
				IBeacon iBeacon = new IBeacon();
				iBeacon.major = 0;
				iBeacon.minor = 0;
				iBeacon.proximityUuid = "00000000-0000-0000-0000-000000000000";
		...
		if (patternFound == false) {
			...
			if (IBeaconManager.LOG_DEBUG)
				Log.d(TAG,
						"This is not an iBeacon advertisment (no 4c000215 seen in bytes 2-5).  The bytes I see are: "
								+ bytesToHex(scanData));
			return null;
		}
	...
	protected IBeacon(String proximityUuid, int major, int minor, int txPower,
			int rssi) {
